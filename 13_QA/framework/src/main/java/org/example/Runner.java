...
public class Runner {
    ...
    public static void run(String packageName) {
        ...
        Set<Class<?>> classes = reflections.getTypesAnnotatedWith(RunWithMyTestFrameworkRunner.class);
        for (Class<?> clazz : classes) {
            Reflections classReflections = new Reflections(clazz, Scanners.MethodsAnnotated);
            invokeMethods(clazz, classReflections, Before.class);
            invokeMethods(clazz, classReflections, Test.class);
            invokeMethods(clazz, classReflections, After.class);
        }
        System.out.println("\nTests passed: "+ testsPassed +", Tests failed: "+ testsFailed);
    }

    private static void invokeMethods(Class<?> clazz, Reflections reflections, Class<? extends Annotation> annotationClass) {
